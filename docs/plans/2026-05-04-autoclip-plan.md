...
| 2026-05-05 | v0.4-corr | M2a brainstorming Q5-corr 触发 plan-4 回滚（user_rules 第 7 条"严禁假设实现"反面教材记录）：v0.4 第 ⑥ 项写"M2a.6 用 env var AUTOCLIP_STYLE_PRESET 注入避免 schema 漂移"是基于错误假设——当时未 read 真实 state.py / api/jobs.py，假设 M1.4 没实现 `_JobMeta.style_preset` + `_JobMeta.target_duration_sec` 字段。Q5-corr 用 read_file 校正后发现 M1.4 早在 `state.py:91-98` `_JobMeta` dataclass + `state.py:122-128` `init_state()` + `api/jobs.py:103-107` `POST /api/jobs` Form 字段已完整实现整条链路。env var 路径不仅多余还引入"state.json vs env var 两个事实源"歧义，本次回滚到"M2a.6 直接从 state.json 读取"。同时根据 Q5-corr A+C 决策，文档化"前端 M3.7 Web UI 默认值 = `clamp(video_duration / 10, 30, 600)`"作为 UI 层 fallback 规约（API 层保持必填）。无工时变化（回滚后 M2a.6 反而少改 2 个文件）|
