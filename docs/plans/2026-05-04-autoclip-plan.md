...
| 2026-05-06 | v0.9 | Session 35：接入 Qwen-TTS（替换 Volcengine）+ 声音内容匹配（commit pending）。**brainstorming Q1-Q4 收敛**：Q1=A 完全替换 Volcengine（单 provider 简洁，stub 在 Qwen 内部重建）/ Q2=B `qwen3-tts-instruct-flash` 单模型（instructions 自然语言控制是核心红利，放弃方言音色）/ Q3=B 6 音色 1:1 固定映射（persona_id → voice_id 是纯函数，可预期可审计可灰度）/ Q4=A 描述语义匹配直接拍板。**映射表**：archaeologist→Elias / empathy_senior→Maia / healing_big_sister→Seren / rage_brother→Vincent / sarcastic_gen_z→Vivian / toxic_middle_aged→Eldric Sage / fallback→Ethan。**新增模块**：`src/autoclip/providers/tts/{qwen,voice_map,instructions}.py`（QwenTTSProvider + select_voice + build_instructions 三个纯函数 + dataclass）；**重构**：`base.py` 提取 create_silent_wav / estimate_stub_duration / probe_duration 共享工具 + TTSProvider.synthesize_batch 加 instructions 可选参数；`assembly.py` 切 QwenTTSProvider，从 timeline.json 读 recommended_persona.persona_id + state.json.style_preset，写 assembly.json 加 voice_id/voice_display_name/persona_id/style_preset/instructions 5 个审计字段。**删除**：volcengine.py + Settings.volcengine_tts_token/volcengine_tts_app_id + .env.example VOLCENGINE_TTS_* + tests/unit/test_tts_provider.py。**新增测试**：test_qwen_tts.py (24) + test_voice_map.py (10) + test_instructions_builder.py (29) = 63 个新单测全过；总 505 unit/integration 通过。**design.md 增量**：§25 Qwen-TTS 声音匹配 章节（25.1-25.8 共 124 行：决策树 + 模块拓扑 + 映射表 + instructions 组合规则 + 配置矩阵 + 双层 stub fallback + assembly.json schema 增量 + Consequences）。**双层 stub fallback**：① 无 DASHSCOPE_API_KEY → 全程 silent WAV / ② 单句 API 失败 3 次 retry 后 → 该句 silent WAV，pipeline 永不 FAILED。**配置收敛**：单 DASHSCOPE_API_KEY 同时驱动 LLM + TTS（凭据管理面 -50%）。**顺手修复**：api/jobs.py 补 `from datetime import UTC, datetime`（Session 34 误删 import 致 e2e 4 个 datetime.now(UTC) 失败）。**已知历史回归**（不在本次 scope）：test_list_jobs_returns_newest_first / test_cancel_job_writes_cancel_signal 仍报 KeyError 'job_id'，git stash 验证为 Session 33 引入，需独立 adhoc 修复。**工时**：实际 ~3h（brainstorming 4 轮 → 实现 13 文件 → 验证 3 检查项），相对 M3.1 估时无超支。 |
